...
import { ArrowLeft, Building2, CheckCircle2, ChevronRight, ClipboardList, Loader2, Mail, Plus, Search, ShieldCheck, Trash2, UserRound } from 'lucide-react';
...
const ExternalRegistrationPage: React.FC = () => {
  ...
  const initialMode = window.location.pathname.includes('/status') || (params.get('request') && params.get('token')) ? 'STATUS' : 'FORM';
  const [mode, setMode] = useState<'FORM' | 'RESULT' | 'STATUS'>(initialMode);
  ...
  useEffect(() => {
    if (initialMode !== 'STATUS') return;
    const requestNo = params.get('request');
    const token = params.get('token');
    if (!requestNo || !token) return;
    supabase.rpc('get_external_access_application_status', { request_no_param: requestNo, tracking_token_param: token }).then(({ data, error: statusError }) => {
      if (statusError || !data) setError('ไม่พบข้อมูลคำขอ หรือ Link ติดตามสถานะไม่ถูกต้อง');
      else setStatusData(data);
  ...
  const submit = async (event: React.FormEvent) => {
    event.preventDefault(); setError(''); setEmailNotice('');
    ...
    try {
      ...
      setResult(submission); setMode('RESULT');
      try {
        const emailResponse = await fetch('/api/send-external-registration-submission', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ requestNo: submission.request_no, trackingToken: submission.tracking_token }) });
        if (!emailResponse.ok) setEmailNotice('บันทึกคำขอแล้ว แต่ระบบยังส่ง Email ไม่สำเร็จ Admin สามารถตรวจสอบและส่งซ้ำภายหลังได้');
      } catch { setEmailNotice('บันทึกคำขอแล้ว แต่ระบบยังไม่สามารถส่ง Email ได้'); }
    } catch (err: any) { setError(err?.message || 'ไม่สามารถส่งคำขอได้ กรุณาลองใหม่อีกครั้ง'); }
    finally { setSubmitting(false); }
  };

  const resetForm = () => { setForm({ companyName: '', firstNameTh: '', lastNameTh: '', firstNameEn: '', lastNameEn: '', jobTitle: '', email: '', phone: '', types: [], coordinators: [''], pdpa: false }); setResult(null); setError(''); setEmailNotice(''); setMode('FORM'); };
  ...
  if (featureLoading) return <PageShell><LoadingState label="กำลังตรวจสอบสถานะการเปิดรับคำขอ" /></PageShell>;
  ...
  if (mode === 'STATUS') return <PageShell><StatusView loading={loadingStatus} data={statusData} error={error} onBack={() => { window.location.href = '/external-registration'; }} /></PageShell>;

  return <PageShell>
    <div className="mb-6 flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-700">External Access Request</p><h1 className="mt-1 text-2xl font-black tracking-tight text-slate-900">ลงทะเบียนใช้งานระบบภายนอก</h1><p className="mt-2 text-xs font-bold leading-relaxed text-slate-500">กรอกข้อมูลเพื่อส่งคำขอให้ Admin ตรวจสอบและอนุมัติ</p></div><a href="/" className="flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-[10px] font-black text-slate-700"><ArrowLeft size={15} /> กลับ</a></div>
...
const EmptyState = ({ onBack }: { onBack: () => void }) => <div className="rounded-[2rem] border border-slate-200 bg-white p-8 text-center shadow-sm"><ShieldCheck className="mx-auto text-slate-400" size={36} /><h1 className="mt-4 text-xl font-black text-slate-900">ยังไม่เปิดรับคำขอ</h1><p className="mt-2 text-xs font-bold leading-relaxed text-slate-500">ขณะนี้ระบบยังไม่เปิดรับคำขอลงทะเบียนใช้งานระบบภายนอก กรุณาติดต่อ Admin</p><button onClick={onBack} className="mt-6 min-h-11 rounded-xl bg-slate-900 px-5 text-xs font-black text-white">กลับหน้าหลัก</button></div>;
const ResultView = ({ result, trackingUrl, emailNotice, onNew }: { result: SubmissionResult; trackingUrl: string; emailNotice: string; onNew: () => void }) => <div className="rounded-[2rem] border border-slate-200 bg-white p-6 text-center shadow-sm md:p-10"><CheckCircle2 className="mx-auto text-emerald-600" size={48} /><p className="mt-5 text-[10px] font-black uppercase tracking-[0.2em] text-emerald-700">Request Submitted</p><h1 className="mt-2 text-2xl font-black text-slate-900">ส่งคำขอเรียบร้อยแล้ว</h1><p className="mt-3 text-xs font-bold text-slate-500">เลขที่คำขอของคุณ</p><div className="mt-2 rounded-2xl bg-slate-900 px-5 py-4 text-xl font-black tracking-widest text-white">{result.request_no}</div>{emailNotice ? <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-left text-xs font-bold leading-relaxed text-amber-800">{emailNotice}</div> : <p className="mt-5 text-xs font-bold text-slate-600">ระบบได้ส่ง Email ยืนยันไปยัง Email ที่แจ้งไว้แล้ว</p>}<div className="mt-6 rounded-2xl border border-blue-200 bg-blue-50 p-4 text-left"><p className="text-[10px] font-black uppercase tracking-widest text-blue-700">Secure Tracking Link</p><p className="mt-2 break-all text-[10px] font-bold text-blue-900">{trackingUrl}</p><a href={trackingUrl} className="mt-3 inline-flex min-h-10 items-center gap-2 rounded-xl bg-blue-700 px-4 text-[10px] font-black text-white">ติดตามสถานะคำขอ <Search size={14} /></a></div><button onClick={onNew} className="mt-6 min-h-11 rounded-xl border border-slate-200 px-5 text-xs font-black text-slate-700">ส่งคำขอใหม่</button></div>;
const StatusView = ({ loading, data, error, onBack }: { loading: boolean; data: any; error: string; onBack: () => void }) => <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-10">{loading ? <LoadingState label="กำลังโหลดสถานะคำขอ" /> : error || !data ? <div className="text-center"><p className="text-sm font-black text-red-700">{error || 'ไม่พบข้อมูลคำขอ'}</p><button onClick={onBack} className="mt-6 min-h-11 rounded-xl bg-slate-900 px-5 text-xs font-black text-white">กลับหน้าลงทะเบียน</button></div> : <><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-widest text-blue-700">Request Status</p><h1 className="mt-2 text-2xl font-black text-slate-900">{data.request_no}</h1></div><span className="rounded-full bg-blue-100 px-3 py-1 text-[10px] font-black text-blue-700">{data.status}</span></div><div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2"><Info label="บริษัท" value={data.company_name} /><Info label="Company Status" value={data.company_resolution} /><Info label="วันที่ส่งคำขอ" value={data.submitted_at ? new Date(data.submitted_at).toLocaleString('th-TH') : '-'} /><Info label="ประเภท" value={(data.types || []).map((item: any) => TYPE_LABELS[item.type_code as UserType] || item.type_code).join(', ')} /></div>{data.admin_note && <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs font-bold text-amber-800"><strong>หมายเหตุจาก Admin:</strong><br />{data.admin_note}</div>}{data.rejection_reason && <div className="mt-5 rounded-2xl border border-red-200 bg-red-50 p-4 text-xs font-bold text-red-800"><strong>เหตุผล:</strong><br />{data.rejection_reason}</div>}<button onClick={onBack} className="mt-6 flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 px-4 text-xs font-black text-slate-700"><ArrowLeft size={15} /> กลับหน้าลงทะเบียน</button></>}</div>;
