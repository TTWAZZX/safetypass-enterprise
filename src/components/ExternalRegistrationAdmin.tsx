...
import {
  ...
  Send, ShieldCheck, Trash2, UserRound, X, XCircle,
} from 'lucide-react';
...
const ACTION_LABELS: Record<ResolveAction, string> = {
  ...
  REJECTED: 'ไม่อนุมัติคำขอและส่ง Email',
  NEED_MORE_INFO: 'ส่งคำขอข้อมูลเพิ่มเติมและส่ง Email',
  UNDER_REVIEW: 'บันทึกสถานะกำลังตรวจสอบและส่ง Email',
};
...
const ExternalRegistrationAdmin: React.FC = () => {
  ...
  const openApplication = async (id: string, reset = true) => {
    ...
    try {
      ...
      showToast(err?.message || 'ไม่สามารถโหลดรายละเอียดคำขอได้', 'error');
    } finally {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (selected && rows.some((row) => row.id === selected.application.id)) {
  ...
  const closeDrawer = () => {
    if (!saving && !sendingEmail) setDrawerOpen(false);
  };

  const canReview = Boolean(selected && !['APPROVED', 'REJECTED', 'CANCELLED'].includes(selected.application.status));
  ...
  const requestResolve = (action: ResolveAction) => {
    ...
    if (action === 'REJECTED' && !rejectionReason.trim()) {
      showToast('กรุณาระบุเหตุผลการไม่อนุมัติก่อนดำเนินการ', 'error');
      return;
    }
    if (action === 'NEED_MORE_INFO' && !adminNote.trim()) {
      showToast('กรุณาระบุข้อมูลที่ต้องการให้ผู้สมัครเพิ่มเติม', 'error');
      return;
    }
    if (action === 'APPROVED' && vendorChoice === 'EXISTING' && !vendorId) {
      showToast('กรุณาเลือกบริษัทเดิม หรือเลือกสร้างบริษัทใหม่', 'error');
      return;
    }
  ...
  const resolve = async (action: ResolveAction) => {
    ...
    try {
      ...
      if (['APPROVED', 'REJECTED', 'UNDER_REVIEW', 'NEED_MORE_INFO'].includes(action)) {
        ...
        try {
          ...
          if (!emailResult.success || emailResult.sent === 0) {
            showToast('บันทึกสถานะแล้ว แต่ยังไม่มี Email ที่ส่งออกสำเร็จ', 'warning');
          } else {
            showToast(`บันทึกสถานะและส่ง Email ให้ผู้สมัครแล้ว (${emailResult.sent} ฉบับ)`, 'success');
          }
        } catch (emailError: any) {
          showToast(`บันทึกสถานะแล้ว แต่ส่ง Email ไม่สำเร็จ: ${emailError?.message || 'กรุณาลองส่งซ้ำ'}`, 'warning');
        } finally {
      ...
      showToast(err?.message || 'ไม่สามารถบันทึกผลคำขอได้', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const retryEmail = async () => {
    ...
    try {
      ...
      showToast(err?.message || 'ส่ง Email ซ้ำไม่สำเร็จ', 'error');
    } finally {
      setSendingEmail(false);
    }
  };
  ...
  const deleteRequest = async () => {
    ...
    setSaving(true);
    try {
      await api.deleteExternalRegistrationApplication(selected.application.id, 'ลบโดย Admin ตามการจัดการคำขอ');
      showToast('ลบคำขอออกจากรายการแล้ว โดยเก็บประวัติไว้ในระบบ', 'success');
      setConfirmation(null);
      setDrawerOpen(false);
      setSelected(null);
      await load();
    } catch (err: any) {
      showToast(err?.message || 'ไม่สามารถลบคำขอได้', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const confirmAction = async () => {
    ...
    else if (nextAction.kind === 'DELETE') await deleteRequest();
    else await resolve(nextAction.action);
  };

  if (loading && applications.length === 0) {
    return <AsyncState variant="loading" title="กำลังโหลดคำขอลงทะเบียน" description="กำลังตรวจสอบสิทธิ์และดึงข้อมูลสำหรับ Admin" />;
  }
  if (error && applications.length === 0) {
    return <AsyncState variant="error" title="โหลดรายการคำขอไม่สำเร็จ" description={error} onRetry={load} />;
  }

  return <div className="space-y-6 animate-in fade-in duration-500">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
      <div className="flex items-start gap-3"><div className="rounded-2xl bg-blue-700 p-3 text-white shadow-lg"><ClipboardList size={24} /></div><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-700">External Registration</p><h2 className="mt-1 text-2xl font-black tracking-tight text-slate-900">คำขอลงทะเบียน Contractor Online / Supplier E-Pass</h2><p className="mt-1 text-xs font-bold text-slate-500">ตรวจสอบบริษัท จัดการสถานะ และแจ้งผลผู้สมัครในหน้าต่างเดียว</p></div></div>
    ...
    {drawerOpen && <div className="fixed inset-0 z-[140] flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm md:p-8" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeDrawer(); }}><aside className="flex h-[88vh] max-h-[900px] w-full max-w-3xl flex-col overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-2xl ring-1 ring-black/10" role="dialog" aria-modal="true" aria-labelledby="external-request-dialog-title"><div className="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 bg-white px-5 py-4 md:px-7"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-700">ตรวจสอบคำขอ</p><h2 id="external-request-dialog-title" className="mt-1 text-lg font-black text-slate-900">รายละเอียดคำขอ</h2><p className="mt-1 text-[10px] font-bold text-slate-500">ตรวจสอบข้อมูลและดำเนินการในหน้าต่างนี้</p></div><button type="button" onClick={closeDrawer} disabled={saving || sendingEmail} aria-label="ปิดรายละเอียดคำขอ" className="flex min-h-11 min-w-11 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 disabled:opacity-50"><X size={20} /></button></div><div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">{detailLoading || !selected ? <div className="flex min-h-80 items-center justify-center gap-2 text-xs font-bold text-slate-500"><Loader2 size={18} className="animate-spin" /> กำลังโหลดรายละเอียดคำขอ</div> : <DetailPanel detail={selected} canReview={canReview} vendors={vendorOptions} vendorChoice={vendorChoice} setVendorChoice={setVendorChoice} vendorId={vendorId} setVendorId={setVendorId} newCompanyStatus={newCompanyStatus} setNewCompanyStatus={setNewCompanyStatus} adminNote={adminNote} setAdminNote={setAdminNote} rejectionReason={rejectionReason} setRejectionReason={setRejectionReason} saving={saving || sendingEmail} onRequestResolve={requestResolve} onRequestRetryEmail={() => setConfirmation({ kind: 'RETRY_EMAIL' })} onRequestDelete={() => setConfirmation({ kind: 'DELETE' })} />}</div></aside></div>}
...
const DetailPanel = (props: {
  ...
  saving: boolean; onRequestResolve: (action: ResolveAction) => void; onRequestRetryEmail: () => void; onRequestDelete: () => void;
}) => {
  const { detail } = props;
  ...
  return <div className="space-y-6 p-5 pb-8 md:p-7">
    ...
    {props.canReview ? <section className="space-y-4 rounded-2xl border border-blue-200 bg-blue-50/60 p-4"><div><SectionHeading icon={<ShieldCheck size={15} />} title="จัดการบริษัทและผลคำขอ" /><p className="mt-2 text-[10px] font-bold leading-relaxed text-slate-600">ตรวจสอบข้อมูลก่อนเลือกบริษัทและผลคำขอ ระบบจะบันทึกประวัติและส่ง Email ตามสถานะที่เลือก</p></div><div className="grid grid-cols-2 gap-2" role="group" aria-label="รูปแบบการจัดการบริษัท"><button type="button" onClick={() => props.setVendorChoice('EXISTING')} className={`rounded-xl border p-3 text-left text-[10px] font-black transition ${props.vendorChoice === 'EXISTING' ? 'border-blue-500 bg-white text-blue-700 ring-2 ring-blue-500/10' : 'border-slate-200 bg-white text-slate-600 hover:border-blue-300'}`}>ใช้บริษัทเดิม<p className="mt-1 text-[9px] font-bold text-slate-400">เลือกจากบริษัทที่มีอยู่</p></button><button type="button" onClick={() => props.setVendorChoice('NEW')} className={`rounded-xl border p-3 text-left text-[10px] font-black transition ${props.vendorChoice === 'NEW' ? 'border-blue-500 bg-white text-blue-700 ring-2 ring-blue-500/10' : 'border-slate-200 bg-white text-slate-600 hover:border-blue-300'}`}>สร้างบริษัทใหม่<p className="mt-1 text-[9px] font-bold text-slate-400">ใช้เมื่อไม่พบบริษัท</p></button></div>{props.vendorChoice === 'EXISTING' ? <label className="block text-[10px] font-black text-slate-600">บริษัทเดิม<select value={props.vendorId} onChange={(e) => props.setVendorId(e.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700 outline-none focus:border-blue-500"><option value="">-- เลือกบริษัท --</option>{props.vendors.map((vendor) => <option key={vendor.id} value={vendor.id}>{vendor.name} ({vendor.status})</option>)}</select></label> : <label className="block text-[10px] font-black text-slate-600">สถานะบริษัทใหม่<select value={props.newCompanyStatus} onChange={(e) => props.setNewCompanyStatus(e.target.value as 'PENDING' | 'APPROVED')} className="mt-2 min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700 outline-none focus:border-blue-500"><option value="PENDING">PENDING — รอตรวจสอบบริษัท</option><option value="APPROVED">APPROVED — ยืนยันบริษัทแล้ว</option></select></label>}<label className="block text-[10px] font-black text-slate-600">หมายเหตุ Admin<textarea value={props.adminNote} onChange={(e) => props.setAdminNote(e.target.value)} rows={3} className="mt-2 w-full rounded-xl border border-slate-200 bg-white p-3 text-xs font-bold text-slate-700 outline-none focus:border-blue-500" placeholder="พิมพ์หมายเหตุหรือข้อมูลที่ต้องการแจ้งผู้สมัคร" /></label><label className="block text-[10px] font-black text-slate-600">เหตุผลกรณีไม่อนุมัติ<textarea value={props.rejectionReason} onChange={(e) => props.setRejectionReason(e.target.value)} rows={3} className="mt-2 w-full rounded-xl border border-slate-200 bg-white p-3 text-xs font-bold text-slate-700 outline-none focus:border-red-500" placeholder="ต้องกรอกเมื่อเลือกไม่อนุมัติ" /></label><div className="grid grid-cols-1 gap-2 sm:grid-cols-2"><ActionButton variant="neutral" onClick={() => props.onRequestResolve('UNDER_REVIEW')} disabled={props.saving}>บันทึกกำลังตรวจสอบ</ActionButton><ActionButton variant="purple" onClick={() => props.onRequestResolve('NEED_MORE_INFO')} disabled={props.saving}>ขอข้อมูลเพิ่มเติม</ActionButton><ActionButton variant="danger" icon={<XCircle size={15} />} onClick={() => props.onRequestResolve('REJECTED')} disabled={props.saving}>ไม่อนุมัติและส่ง Email</ActionButton><ActionButton variant="success" icon={<CheckCircle2 size={15} />} onClick={() => props.onRequestResolve('APPROVED')} disabled={props.saving}>อนุมัติและส่ง Email</ActionButton></div><p className="text-center text-[9px] font-bold text-slate-500">ระบบจะแสดงหน้าต่างยืนยันก่อนบันทึกผลทุกครั้ง</p></section> : <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-start gap-3"><CheckCircle2 size={18} className={application.status === 'APPROVED' ? 'text-emerald-600' : 'text-red-600'} /><div><p className="text-sm font-black text-slate-700">คำขอนี้ดำเนินการแล้ว</p><p className="mt-1 text-[10px] font-bold leading-relaxed text-slate-500">ระบบบันทึกผลและประวัติเรียบร้อยแล้ว หากผู้สมัครไม่ได้รับ Email สามารถส่งซ้ำได้</p></div></div><button type="button" onClick={props.onRequestRetryEmail} disabled={props.saving} className="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-blue-200 bg-white px-3 text-[10px] font-black text-blue-700 hover:bg-blue-50 disabled:opacity-50"><Send size={14} /> ส่ง Email ผลลัพธ์ซ้ำ</button></section>}
    ...
    {application.status !== 'CANCELLED' && <button type="button" onClick={props.onRequestDelete} disabled={props.saving} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-red-200 bg-red-50 px-4 text-xs font-black text-red-700 hover:bg-red-100 disabled:opacity-50"><Trash2 size={15} /> ลบคำขอนี้</button>}
  </div>;
};
...
const ConfirmationDialog = ({ request, application, vendorChoice, vendorName, newCompanyStatus, rejectionReason, saving, onCancel, onConfirm }: { request: ConfirmationRequest; application: ExternalRegistrationApplicationRow; vendorChoice: 'EXISTING' | 'NEW'; vendorName: string; newCompanyStatus: 'PENDING' | 'APPROVED'; rejectionReason: string; saving: boolean; onCancel: () => void; onConfirm: () => void }) => {
  ...
  const title = isDelete ? 'ยืนยันการลบคำขอ' : isRetry ? 'ยืนยันการส่ง Email ซ้ำ' : `ยืนยัน${action ? ACTION_LABELS[action] : 'การดำเนินการ'}`;
  const confirmLabel = isDelete ? 'ยืนยันลบคำขอ' : isRetry ? 'ยืนยันส่ง Email ซ้ำ' : action === 'APPROVED' ? 'ยืนยันอนุมัติและส่ง Email' : action === 'REJECTED' ? 'ยืนยันไม่อนุมัติและส่ง Email' : `ยืนยัน${action ? ACTION_LABELS[action] : 'การดำเนินการ'}`;
  const icon = isDelete ? <Trash2 size={22} /> : action === 'REJECTED' ? <XCircle size={22} /> : action === 'APPROVED' ? <CheckCircle2 size={22} /> : <ShieldCheck size={22} />;
  return <div className="fixed inset-0 z-[160] flex items-center justify-center bg-slate-950/65 p-4 backdrop-blur-sm" role="presentation"><div className="w-full max-w-lg rounded-[2rem] border border-white/20 bg-white p-6 shadow-2xl md:p-7" role="dialog" aria-modal="true" aria-labelledby="external-confirm-title"><div className="flex items-start gap-4"><div className={`rounded-2xl p-3 ${isDelete || action === 'REJECTED' ? 'bg-red-100 text-red-700' : action === 'APPROVED' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'}`}>{icon}</div><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-widest text-slate-500">ยืนยันการดำเนินการ</p><h2 id="external-confirm-title" className="mt-1 text-lg font-black text-slate-900">{title}</h2><p className="mt-2 text-xs font-bold leading-relaxed text-slate-600">กรุณาตรวจสอบข้อมูลอีกครั้ง การดำเนินการนี้จะถูกบันทึกไว้ในประวัติคำขอ</p></div></div><div className="mt-5 space-y-2 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs"><SummaryRow label="เลขคำขอ" value={application.request_no} /><SummaryRow label="บริษัท" value={application.company_name_submitted} /><SummaryRow label="ผู้สมัคร" value={`${application.first_name_th} ${application.last_name_th}`} /><SummaryRow label="Email" value={application.login_email} />{action === 'APPROVED' && <SummaryRow label="การจัดการบริษัท" value={vendorChoice === 'EXISTING' ? `ใช้บริษัทเดิม${vendorName ? `: ${vendorName}` : ''}` : `สร้างบริษัทใหม่ (${newCompanyStatus})`} />}{action === 'REJECTED' && <SummaryRow label="เหตุผล" value={rejectionReason} />}{isDelete && <p className="mt-3 border-t border-slate-200 pt-3 text-[10px] font-bold text-red-700">คำขอจะถูกซ่อนจากรายการปกติ แต่ข้อมูลและประวัติจะยังเก็บไว้ในระบบ</p>}{isRetry && <p className="mt-3 border-t border-slate-200 pt-3 text-[10px] font-bold text-amber-700">ระบบจะส่ง Email ผลลัพธ์ซ้ำไปยังผู้สมัครตาม Email นี้</p>}{action && <p className="mt-3 border-t border-slate-200 pt-3 text-[10px] font-bold text-blue-700">ระบบจะส่ง Email แจ้งสถานะไปยังผู้สมัครหลังบันทึกสำเร็จ</p>}</div><div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={onCancel} disabled={saving} className="min-h-11 rounded-xl border border-slate-200 px-5 text-xs font-black text-slate-700 hover:bg-slate-50 disabled:opacity-50">ยกเลิก</button><button type="button" onClick={onConfirm} disabled={saving} className={`flex min-h-11 items-center justify-center gap-2 rounded-xl px-5 text-xs font-black text-white disabled:opacity-50 ${isDelete || action === 'REJECTED' ? 'bg-red-600 hover:bg-red-700' : action === 'APPROVED' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-blue-700 hover:bg-blue-800'}`}>{saving && <Loader2 size={15} className="animate-spin" />}{confirmLabel}</button></div></div></div>;
};

const SummaryRow = ({ label, value }: { label: string; value: string }) => <div className="grid grid-cols-[100px_minmax(0,1fr)] gap-3"><span className="font-bold text-slate-500">{label}</span><span className="break-words font-black text-slate-800">{value}</span></div>;
