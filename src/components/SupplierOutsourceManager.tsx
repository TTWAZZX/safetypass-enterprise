  ...
  return (
    <div className="space-y-6 pb-12 text-left">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div><h2 className="text-2xl font-black uppercase text-slate-900">Supplier & Outsource</h2><p className="text-[10px] font-bold uppercase tracking-widest text-slate-600">Program Control & Reporting</p></div>
        <div className="flex flex-wrap gap-2">
          <button onClick={() => openForm()} className="flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-emerald-700 px-4 py-3 text-[9px] font-black uppercase tracking-widest text-white hover:bg-emerald-800 sm:flex-none"><Plus size={16}/> เพิ่มสิทธิ์</button>
      ...
      <div className={`flex flex-col justify-between gap-4 rounded-2xl border p-4 sm:flex-row sm:items-center ${launchStatus.enabled ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50'}`}>
        ...
        <button onClick={toggleFeature} disabled={!launchStatus.enabled && launchStatus.activeQuestionCount < 20} className={`min-h-11 rounded-xl px-4 py-3 text-[9px] font-black uppercase tracking-widest text-white disabled:cursor-not-allowed disabled:bg-slate-600 ${launchStatus.enabled ? 'bg-amber-800' : 'bg-emerald-700'}`}>{launchStatus.enabled ? 'ปิดโปรแกรม' : 'เปิดโปรแกรม'}</button>
      </div>

      <div className="grid grid-cols-2 gap-3 md:grid-cols-5 xl:grid-cols-10">
        {Object.entries(stats).map(([key, value]) => <div key={key} className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm"><p className="text-[8px] font-black uppercase tracking-widest text-slate-600">{key.replace('_', ' ')}</p><p className="mt-1 text-2xl font-black text-slate-900">{value}</p></div>)}
      </div>
                    ...
                    return (
                      <article key={row.user_id} className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm" aria-label={`สิทธิ์ Supplier & Outsource ของ ${row.name}`}>
                        <div className="flex items-start justify-between gap-3 border-b border-slate-100 pb-3">
                          <div className="min-w-0"><h3 className="truncate text-sm font-black text-slate-900">{row.name}</h3><p className="mt-1 truncate text-[9px] font-bold text-slate-600">{row.company || 'ไม่มีสังกัด'}</p></div>
                        ...
                        <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-3 text-[10px]">
                          <div><dt className="font-black text-slate-600">ประเภทผู้ใช้</dt><dd className="mt-1 font-bold capitalize text-slate-700">{row.participant_type || '-'}</dd></div>
                          <div><dt className="font-black text-slate-600">ลักษณะงาน</dt><dd className="mt-1 font-bold text-slate-700">{row.work_type || '-'}</dd></div>
                          <div><dt className="font-black text-slate-600">คะแนน</dt><dd className="mt-1 text-sm font-black text-emerald-700">{row.score ?? '-'} / {row.total_questions ?? '-'}</dd></div>
                          <div><dt className="font-black text-slate-600">วันที่สอบ</dt><dd className="mt-1 font-bold text-slate-700">{row.test_date ? new Date(row.test_date).toLocaleDateString('th-TH') : '-'}</dd></div>
                          <div className="col-span-2"><dt className="font-black text-slate-600">วันหมดอายุ</dt><dd className="mt-1 font-bold text-slate-700">{row.expiration_date ? new Date(row.expiration_date).toLocaleDateString('th-TH') : '-'}</dd></div>
                        </dl>
